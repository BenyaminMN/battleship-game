...
def place_and_print_ships(array, quantity_ships, player):
    # We divide and round down to an integer (since we cannot place a non-integer part of a ship)
...
def request_coordinates(player):
    ...
    while True:
        ...
        # We need a letter of 1 character. If it is not 1 character we use continue to repeat this cycle
        if len(letter_row) != 1:
...
def all_sunk_ships(array):
    for y in range(ROWS):
        for x in range(COLUMNS):
            cell = array[y][x]
            # If it is not sea or a shot, it means there's still a ship out there
